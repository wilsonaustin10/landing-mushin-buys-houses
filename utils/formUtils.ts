...
const lastSubmissionTime: { [key: string]: number } = {};

// Save form data to localStorage
// const saveToLocalStorage = (formId: string, data: Partial<FormData>): boolean => { // Unused
//   try {
...
// const generateDataKey = (formData: Partial<FormData>): string => { // Unused
//   const relevantData = {
...
// const trackPartialSubmission = (formData: Partial<FormData>): void => { // Unused
//   try {
...
const trackFormInteraction = (formData: Partial<FormData>, action: string): void => {
  try {
    ...
    if (typeof window !== 'undefined' && window.hj) {
      ...
      window.hj('trigger', 'form_interaction');
    }
  } catch (error) {
...
export const autoSaveForm = debounce(async (formId: string, formData: Partial<FormData>): Promise<void> => {
  ...
  // const savedLocally = saveToLocalStorage(formId, formData); // Unused
...
// const PARTIAL_LEAD_TIMEOUT = 60000; // 1 minute - Unused
...
// function calculateLeadScore(formState: FormState): number { // Unused
//   let score = 0;
//   
//   // Core fields
...
// function calculateDynamicTimeout(formState: FormState): number { // Unused
